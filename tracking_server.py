...
def track_open():
    ...
    # 🚀 Block automated pre-fetching bots (Gmail/Outlook pre-caching)
    bot_keywords = ["googleimageproxy", "yahoo! slurp", "bingpreview", "bot", "spider", "crawler", "yahooimageproxy"]
    if any(keyword in user_agent for keyword in bot_keywords):
        print(f"🤖 Blocked automated pre-fetch from: {user_agent}")
    ...
    if email:
        try:
            ...
            conn.commit()
            conn.close()
            print(f"🔥 Real open recorded for: {email}")
...
def send_blank_pixel():
    ...
    # Fallback to a 1x1 transparent tracking GIF if pixel.png doesn't exist
    import base64
    return base64.b64decode(b'R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7'), 200, {'Content-Type': 'image/gif'}
